...
void Dialog::startApp()
{
    ...
    //m_appname必须是ASCII，不能包含中文，否则用::ShellExecuteW 和m_appname.toStdWString()
    ::ShellExecuteA(0,QString("open").toStdString().c_str(),m_appname.toStdString().c_str(),
                    QString("").toStdString().c_str(),QString("").toStdString().c_str(),1);
    m_ui->TextEdit_Info->appendHtml((QTime::currentTime()).toString("hh:mm:ss")
                                    +" start "+m_appname+"\n");
...
{
    QDir::setCurrent(m_appdir);
    ::system(QString("taskkill /F /IM "+m_appname).toStdString().c_str());
    m_ui->TextEdit_Info->appendHtml((QTime::currentTime()).toString("hh:mm:ss")
                                    +" kill "+m_appname+"\n");
}
